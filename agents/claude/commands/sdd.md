...
3. tasks.md を作成
   ...
   - 動作確認をしやすくするため、ユーザーに近い側から実装する順番にタスクを組む。DDD アーキテクチャを採用している場合は Presentation, Domain, Service 層から実装する。その際、実際の Repositroy の代わりにダミーデータやモック Repository を用いて実装する。最後に Infrastructure 層（Repository の実装を含む）を実装する。例えば、 SwiftUI の iOS アプリであれば View から、 Go の API であれば Controller (Handler) から実装する。
   - Infrastructure や UseCase を実装する際、呼び出し側のダミーを差し替える作業も同じタスクに含める。
   - 各ステップの依存関係を明記
   - チェックボックス `[ ]` で進捗管理
   ...
   - 動作確認はわざわざステップにせず、各ステップ実装後に毎回行う。
